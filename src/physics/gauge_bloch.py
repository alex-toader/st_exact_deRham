...
# ============================================================
# Internal tests — run with: python3 -m physics.gauge_bloch
# ============================================================

def _test_init():
    """Verify core gauge_bloch functionality on Kelvin N=2."""
    from .hodge import build_kelvin_with_dual_info, build_hodge_stars_voronoi

    print("gauge_bloch internal tests")
    print("-" * 40)
    ...
    print(f"  Kelvin N=2: V={nV}, E={nE}, F={len(F)}")

    # T1: edge shifts
    shifts = compute_edge_shifts(V, E, L_vec)
    assert shifts.shape == (nE, 3), f"shifts shape {shifts.shape}"
    print(f"  T1 shifts: shape OK, max|shift|={np.max(np.abs(shifts))}")

    # T2: d0_bloch construction
    k = np.array([0.3, 0.2, 0.1])
    d0k = build_d0_bloch(V, E, k, L_vec, shifts)
    assert d0k.shape == (nE, nV), f"d0k shape {d0k.shape}"
    print(f"  T2 d0_bloch: shape ({nE}, {nV}) OK")

    # T3: d1_bloch_exact + exactness d₁d₀=0
    d1k = build_d1_bloch_exact(V, E, F, k, L_vec, d0k)
    nF = len(F)
    assert d1k.shape == (nF, nE), f"d1k shape {d1k.shape}"
    residual = np.linalg.norm(d1k @ d0k)
    print(f"  T3 d1_bloch_exact: shape ({nF}, {nE}), ||d1*d0||={residual:.2e}")
    assert residual < 1e-10, f"Exactness failed: {residual:.2e}"

    # T4: d1 entries are unimodular (|entry| = 0 or 1)
    nonzero = np.abs(d1k)[np.abs(d1k) > 1e-14]
    max_dev = np.max(np.abs(nonzero - 1.0))
    print(f"  T4 unimodular: max||d1|-1| = {max_dev:.2e}")
    assert max_dev < 1e-10, f"Not unimodular: {max_dev:.2e}"

    # T5: Maxwell eigenvalues — n_zero = nV, c² ≈ 1
    S2 = np.diag(star2)
    M = np.diag(star1)
    K = d1k.conj().T @ S2 @ d1k
    K = 0.5 * (K + K.conj().T)
    ...
    thr = max(max_eig * 1e-12, 1e-14)
    n_zero = np.sum(np.abs(eigvals) < thr)
    print(f"  T5 n_zero={n_zero}, nV={nV}, match={'OK' if n_zero == nV else 'FAIL'}")
    assert n_zero == nV, f"n_zero={n_zero} != nV={nV}"

    # c² from first physical eigenvalue
    k2 = np.dot(k, k)
    phys = eigvals[np.abs(eigvals) >= thr]
    c2 = phys[0] / k2
    print(f"  T5 c²={c2:.6f} (expect ~1.0)")
    assert abs(c2 - 1.0) < 0.05, f"c²={c2:.4f} too far from 1.0"

    # T6: k=0 reduction — d1(k=0) should be real
    k0 = np.array([0.0, 0.0, 0.0])
    d0_k0 = build_d0_bloch(V, E, k0, L_vec, shifts)
    d1_k0 = build_d1_bloch_exact(V, E, F, k0, L_vec, d0_k0)
    imag_norm = np.linalg.norm(np.imag(d1_k0))
    print(f"  T6 d1(k=0) imaginary norm = {imag_norm:.2e}")
    assert imag_norm < 1e-12, f"d1(k=0) not real: {imag_norm:.2e}"
