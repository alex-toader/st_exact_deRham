...
# canonical_face REMOVED — was dead code, different from spec/structures.py
# version (didn't return orientation). Use spec/structures.canonical_face instead.
...
def _test_init():
    """Hodge stars and foam builders test on Kelvin N=2 (~1s)."""
    print("hodge init tests")
    print("-" * 40)

    # T1: Build all 3 structures
    for name, builder in [
        ...
        # T2: Plateau structure
        plateau = verify_plateau_structure(data)
        assert plateau['plateau_edges'], f"{name}: not all edges bound 3 faces"
        assert plateau['plateau_vertices'], f"{name}: not all vertices deg 4"
        assert plateau['dual_orthogonality_mean'] > 0.99, \
            f"{name}: orthogonality {plateau['dual_orthogonality_mean']:.4f}"
        assert plateau['all_ok'], f"{name}: Plateau check failed"

        # T3: Voronoi property (equidistance)
        voronoi = verify_voronoi_property(data)
        assert voronoi['voronoi_ok'], \
            f"{name}: Voronoi asymmetry {voronoi['max_asymmetry']:.2e}"

        # T4: Hodge stars — all positive
        star1, star2 = build_hodge_stars_voronoi(data)
        assert len(star1) == nE, f"{name}: star1 len {len(star1)} != nE={nE}"
        assert len(star2) == nF, f"{name}: star2 len {len(star2)} != nF={nF}"
        assert np.all(star1 > 0), f"{name}: star1 has non-positive values"
        assert np.all(star2 > 0), f"{name}: star2 has non-positive values"

        # T5: star1 ~ L (scales with edge length), star2 ~ 1/L
        L = data['L']
        mean_s1 = np.mean(star1)
        mean_s2 = np.mean(star2)
        # Just check they're in reasonable range (not zero, not huge)
        assert 0.01 < mean_s1 < 100, f"{name}: mean star1 = {mean_s1}"
        assert 0.01 < mean_s2 < 100, f"{name}: mean star2 = {mean_s2}"

        print(f"  {name}: V={nV}, E={nE}, F={nF}, Plateau OK, "
              f"Voronoi OK, stars positive OK")
