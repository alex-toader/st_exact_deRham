...
def _test_init():
    """Bloch operator tests on Kelvin N=2 (~1s)."""
    from .hodge import build_kelvin_with_dual_info

    print("bloch init tests")
    print("-" * 40)
    ...
    assert crossings.shape == (nE, 3), f"crossings shape {crossings.shape}"
    assert np.max(np.abs(crossings)) <= 1, "crossings > 1"
    print(f"  T1 crossings: shape ({nE},3), max|n|={np.max(np.abs(crossings))} OK")

    # T2: d0_bloch at k=0 should equal topological d0
    from core_math.operators.incidence import build_d0 as build_d0_topo
    d0_topo = build_d0_topo(V, E)
    k0 = np.array([0.0, 0.0, 0.0])
    d0_k0 = build_d0_bloch(V, E, L, k0, crossings)
    diff = np.max(np.abs(d0_k0 - d0_topo))
    assert diff < 1e-14, f"d0(k=0) != d0_topo: {diff:.2e}"
    print(f"  T2 d0(k=0) = d0_topo: diff={diff:.2e} OK")

    # T3: DisplacementBloch — Hermitian
    db = DisplacementBloch(V, E, L, k_L=3.0, k_T=1.0)
    k_test = np.array([0.1, 0.05, 0.02])
    herm_err = db.check_hermitian(k_test)
    assert herm_err < 1e-12, f"D(k) not Hermitian: {herm_err:.2e}"
    print(f"  T3 D(k) Hermitian: ||D-D†||={herm_err:.2e} OK")

    # T4: DisplacementBloch — 3 acoustic modes at small k
    freqs = db.frequencies(k_test)
    # First 3 should be small (acoustic), rest should be larger (optical)
    assert freqs[0] >= 0, f"Negative frequency: {freqs[0]}"
    acoustic = freqs[:3]
    optical = freqs[3:6]
    assert np.max(acoustic) < np.min(optical), \
        f"Acoustic-optical gap missing: acoustic={acoustic}, optical={optical}"
    print(f"  T4 acoustic/optical: {acoustic[0]:.4f},{acoustic[1]:.4f},{acoustic[2]:.4f} | "
          f"{optical[0]:.4f} OK")

    # T5: T/L classification — with k_L ≠ k_T, should separate
    omega_T, omega_L, f_L = db.classify_modes(k_test)
    assert len(omega_T) == 2, f"Expected 2 T modes, got {len(omega_T)}"
    assert len(omega_L) == 1, f"Expected 1 L mode, got {len(omega_L)}"
    # L should have higher f_L than T
    idx_L = np.argmax(f_L)
    idx_T = [i for i in range(3) if i != idx_L]
    assert f_L[idx_L] > max(f_L[idx_T[0]], f_L[idx_T[1]]), \
        f"L not most longitudinal: f_L={f_L}"
    print(f"  T5 T/L classification: f_L={f_L}, L at idx {idx_L} OK")

    # T6: D(k=0) should have 3 zero eigenvalues (translations)
    eigs_gamma = db.eigenvalues(np.array([0.0, 0.0, 0.0]))
    n_zero = np.sum(np.abs(eigs_gamma) < ZERO_EIGENVALUE_THRESHOLD)
    assert n_zero == 3, f"D(Γ) has {n_zero} zero modes, expected 3"
    print(f"  T6 D(Γ): {n_zero} zero modes (translations) OK")

    # T7: BlochComplex deprecated warning
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        bc = BlochComplex(V, E, F, L)
        assert len(w) == 1 and "exactness" in str(w[0].message).lower(), \
            f"BlochComplex should warn about exactness"
    print(f"  T7 BlochComplex deprecation warning OK")

    # T8: crossings consistency — compute_edge_crossings vs compute_edge_geometry
    crossings1 = compute_edge_crossings(V, E, L)
    _, crossings2 = compute_edge_geometry(V, E, L)
    diff_n = np.max(np.abs(crossings1 - crossings2))
    assert diff_n == 0, f"Crossing functions disagree: max diff = {diff_n}"
    print(f"  T8 crossings consistency: compute_edge_crossings == compute_edge_geometry OK")
