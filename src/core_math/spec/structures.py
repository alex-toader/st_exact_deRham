...
def _test_init():
    """Structures contract tests (~instant)."""
    print("structures init tests")
    print("-" * 40)

    # T1: canonical_face — basic cases
    canon, orient = canonical_face([3, 1, 4, 2])
    assert canon[0] == 1, f"canonical doesn't start at min: {canon}"
    assert orient in {1, -1}

    # Same face reversed → same canonical, opposite orientation
    canon2, orient2 = canonical_face([2, 4, 1, 3])
    assert canon2 == canon, f"Reversed face gives different canonical"
    assert orient2 == -orient, f"Reversed orientation not flipped"

    # Rotation → same canonical, same orientation
    canon3, orient3 = canonical_face([1, 4, 2, 3])
    assert canon3 == canon and orient3 == orient, \
        f"Rotation changed canonical or orientation: {canon3} vs {canon}, {orient3} vs {orient}"
    print(f"  T1 canonical_face: rotation/reversal OK")

    # T2: canonical_face rejects < 3 vertices
    try:
        canonical_face([1, 2])
        assert False, "Should reject < 3 vertices"
    except ValueError:
        pass
    print(f"  T2 canonical_face rejects degenerate OK")

    # T3: validate_mesh catches missing fields
    ok, errs = validate_mesh({}, strict=False)
    assert not ok
    assert len(errs) >= len(MeshContract.REQUIRED_FIELDS)
    print(f"  T3 validate_mesh catches missing fields OK")

    # T4: create_mesh produces valid mesh
    verts = np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=float)
    edges = [(0, 1), (0, 2), (0, 3), (1, 2), (1, 3), (2, 3)]
    faces = [[0, 1, 2], [0, 1, 3], [0, 2, 3], [1, 2, 3]]
    mesh = create_mesh(verts, edges, faces, COMPLEX_SURFACE, name="tetra_test")
    assert mesh['n_V'] == 4
    assert mesh['n_E'] == 6
    assert mesh['n_F'] == 4
    assert mesh['faces_per_edge'] == 2
    ok, errs = validate_mesh(mesh, strict=False)
    assert ok, f"Tetra mesh invalid: {errs}"
    print(f"  T4 create_mesh: tetrahedron valid OK")

    # T5: validate catches bad edge
    bad_mesh = dict(mesh)
    bad_mesh['E'] = list(mesh['E']) + [(5, 6)]  # out of bounds
    ok, errs = validate_mesh(bad_mesh, strict=False)
    assert not ok
    print(f"  T5 validate catches bad edge OK")
