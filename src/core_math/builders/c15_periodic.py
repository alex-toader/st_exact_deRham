...
Verified (N=1): V=136, E=272, F=160, C=24, χ=0.
  144 pentagons + 16 hexagons. 16 Z12 + 8 Z16 cells. Plateau valid.
"""
...
def _test_init():
    """Fast init tests — topology invariants on N=1 (~2s)."""
    print("c15_periodic init tests")
    print("-" * 40)

    # T1: point count
    pts = get_c15_points(1)
    assert len(pts) == 24, f"Expected 24 points, got {len(pts)}"
    pts2 = get_c15_points(2)
    assert len(pts2) == 24 * 8, f"Expected 192, got {len(pts2)}"
    print(f"  T1 point count: N=1→{len(pts)}, N=2→{len(pts2)} OK")

    # T2: build and topology
    verts, edges, faces, cfi = build_c15_supercell_periodic(1)
    V, E, F, C = len(verts), len(edges), len(faces), 24
    print(f"  T2 build: V={V}, E={E}, F={F}, C={C}")
    assert V - E + F - C == 0, f"χ={V-E+F-C} != 0"
    print(f"  T2 χ(3-complex) = 0 OK")

    # T3: Plateau structure
    result = verify_c15_foam_structure(1)
    ...
    print(f"  T3 Plateau: 3 faces/edge, deg 4 OK")

    # T4: face sizes — only pentagons and hexagons
    sizes = set(result['face_sizes'].keys())
    assert sizes <= {5, 6}, f"Unexpected face sizes: {sizes}"
    print(f"  T4 face sizes: {result['face_sizes']} OK")

    # T5: cell face counts — Z12 (12 faces) and Z16 (16 faces)
    face_counts = [len(cell) for cell in cfi]
    unique_counts = set(face_counts)
    assert unique_counts <= {12, 16}, f"Unexpected: {unique_counts}"
    n12 = sum(1 for c in face_counts if c == 12)
    n16 = sum(1 for c in face_counts if c == 16)
    assert n12 == 16 and n16 == 8, f"Expected 16 Z12 + 8 Z16, got {n12}+{n16}"
    print(f"  T5 cells: {n12} Z12 + {n16} Z16 OK")

    # T6: edges canonical (i < j), no duplicates
    for i, j in edges:
        assert i < j, f"Edge ({i},{j}) not canonical"
    assert len(set(edges)) == len(edges), "Duplicate edges"
    print(f"  T6 edges canonical, no duplicates OK")

    # T7: each face in exactly 2 cells, orientations antisymmetric
    face_orients = defaultdict(list)
    for ci, cell in enumerate(cfi):
        for f_idx, orient in cell:
            assert 0 <= f_idx < F, f"Cell {ci}: invalid face {f_idx}"
    ...
    print(f"  T7 face-cell incidence: 2 cells/face, antisymmetric OK")
...
def _test_runtime():
    """Slower runtime tests — N=2 scaling (~20s)."""
    print("\nc15_periodic runtime tests")
    print("-" * 40)

    # R1: N=2 topology
    result = verify_c15_foam_structure(2)
    assert result['chi_3complex'] == 0, f"N=2 χ={result['chi_3complex']}"
    assert result['is_valid_plateau_foam']
    assert result['C'] == 192
    print(f"  R1 N=2: V={result['V']}, E={result['E']}, F={result['F']}, "
          f"C=192, χ=0, Plateau OK")

    # R2: 8× volume scaling N=1→N=2
    r1 = get_c15_periodic_topology(1)
    r2 = get_c15_periodic_topology(2)
    assert r2['V'] == 8 * r1['V'], f"V scaling: {r2['V']} != 8×{r1['V']}"
    assert r2['E'] == 8 * r1['E'], f"E scaling: {r2['E']} != 8×{r1['E']}"
    assert r2['F'] == 8 * r1['F'], f"F scaling: {r2['F']} != 8×{r1['F']}"
    print(f"  R2 scaling: V,E,F all 8× from N=1→N=2 OK")

    print("-" * 40)
    print("ALL RUNTIME PASS")
