...
def build_vertices() -> Tuple[np.ndarray, Dict[tuple, int]]:
    ...
    CONSTRUCTION:
        ...
        Result: 24 unique vertices (after deduplication).
...
def _test_init():
    """Kelvin cell topology tests (~instant)."""
    print("kelvin init tests")
    print("-" * 40)

    topo = get_topology_numbers()
    assert topo['V'] == 24, f"V={topo['V']}"
    assert topo['E'] == 36, f"E={topo['E']}"
    assert topo['F'] == 14, f"F={topo['F']}"
    assert topo['chi'] == 2, f"χ={topo['chi']}"
    print(f"  T1 topology: V=24, E=36, F=14, χ=2 OK")

    vertices, edges, faces, _ = build_kelvin_cell()
    n_sq = sum(1 for f in faces if len(f) == 4)
    n_hex = sum(1 for f in faces if len(f) == 6)
    assert n_sq == 6 and n_hex == 8, f"Faces: {n_sq} sq + {n_hex} hex"
    print(f"  T2 faces: 6 squares + 8 hexagons OK")

    # T3: all vertices degree 3 (surface, not foam)
    ...
    for i, j in edges:
        ...
        deg[j] += 1
    degs = set(deg.values())
    assert degs == {3}, f"Vertex degrees: {degs}"
    print(f"  T3 vertex degree: all 3 OK")

    # T4: edges canonical
    for i, j in edges:
        assert i < j, f"Edge ({i},{j}) not canonical"
    print(f"  T4 edges canonical OK")

    # T5: each edge in exactly 2 faces (closed surface)
    ...
    for face in faces:
        ...
        for k in range(n):
            ...
            edge_face_count[e] += 1
    bad = {e: c for e, c in edge_face_count.items() if c != 2}
    assert not bad, f"Edges not in exactly 2 faces: {bad}"
    print(f"  T5 each edge in 2 faces OK")

    # T6: face vertices distinct
    for i, face in enumerate(faces):
        assert len(set(face)) == len(face), f"Face {i} has repeated vertices"
    print(f"  T6 face vertices unique OK")
