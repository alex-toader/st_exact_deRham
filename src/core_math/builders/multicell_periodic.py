...
def _test_init():
    """BCC multicell periodic init tests (~3s on N=2)."""
    from collections import defaultdict

    print("multicell_periodic init tests")
    print("-" * 40)

    # T1: center count
    for N in [1, 2, 3]:
        c = generate_bcc_centers(N)
        assert len(c) == 2 * N**3, f"N={N}: {len(c)} != {2*N**3}"
    print(f"  T1 center count: 2N³ OK")

    # T2: N=2 topology (N=1 has self-glued faces)
    topo = get_periodic_topology(2)
    assert topo['chi_3complex'] == 0, f"χ(3)={topo['chi_3complex']}"
    assert topo['C'] == 16
    print(f"  T2 N=2: V={topo['V']}, E={topo['E']}, F={topo['F']}, "
          f"C=16, χ=0 OK")

    # T3: Plateau — each edge in 3 faces
    verts, edges, faces, cfi = build_bcc_supercell_periodic(2)
    ...
    for face in faces:
        ...
        for k in range(n):
            ...
            edge_face_count[e] += 1
    bad = {e: c for e, c in edge_face_count.items() if c != 3}
    assert not bad, f"{len(bad)} edges not in 3 faces"
    print(f"  T3 Plateau: all edges in 3 faces OK")
    ...
    for i, j in edges:
        ...
        deg[j] += 1
    degs = set(deg.values())
    assert degs == {4}, f"Vertex degrees: {degs}"
    print(f"  T4 vertex degree: all 4 OK")

    # T5: each face in exactly 2 cells, antisymmetric orientations
    ...
    print(f"  T5 face-cell: 2 cells/face, antisymmetric OK")

    # T6: each cell has 14 faces (Kelvin = 6 sq + 8 hex)
    for ci, cell in enumerate(cfi):
        assert len(cell) == 14, f"Cell {ci} has {len(cell)} faces"
    print(f"  T6 each cell: 14 faces OK")

    # T7: edges canonical, no duplicates
    for i, j in edges:
        assert i < j, f"Edge ({i},{j}) not canonical"
    assert len(set(edges)) == len(edges), "Duplicate edges"
    print(f"  T7 edges canonical, unique OK")

    # T8: build_bcc_foam_periodic(1) raises
    try:
        build_bcc_foam_periodic(1)
        assert False, "Should have raised ValueError"
    except ValueError:
        pass
    print(f"  T8 N=1 raises ValueError OK")
