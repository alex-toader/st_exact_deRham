...
def build_sc_supercell_periodic(N: int) -> Tuple[np.ndarray, List[Tuple[int, int]], List[List[int]], List[List[Tuple[int, int]]]]:
    ...
    for cell_idx, cell_map in enumerate(cell_vertex_map):
        for local_face in base_f:
            ...
            cells = face_data[canonical]['cells']
            if cell_idx in cells:
                assert cells[cell_idx] == rel_orient, \
                    f"Orientation inconsistency: cell {cell_idx}, face {canonical[:3]}..."
            cells[cell_idx] = rel_orient
...
def _test_init():
    """Periodic solid structure tests."""
    from collections import defaultdict

    print("solids_periodic init tests")
    print("-" * 40)

    # SC N=3 (minimum for SC periodic)
    topo = get_sc_periodic_topology(3)
    assert topo['chi_3complex'] == 0, f"SC χ(3)={topo['chi_3complex']}"
    assert topo['C'] == 27
    assert topo['chi_2skeleton'] == topo['C'], "SC χ(2) != C"

    verts, edges, faces, cfi = build_sc_supercell_periodic(3)
    # SC: 4 faces per edge
    efc = defaultdict(int)
    for face in faces:
        n = len(face)
        for k in range(n):
            ...
            efc[e] += 1
    edge_counts = set(efc.values())
    assert edge_counts == {4}, f"SC faces/edge: {edge_counts}, expected {{4}}"

    # Face-cell: each face in exactly 2 cells, antisymmetric
    ...
    for f_idx, orients in face_orients.items():
        assert len(orients) == 2, f"SC face {f_idx} in {len(orients)} cells"
        assert orients[0] + orients[1] == 0, f"SC face {f_idx} not antisymmetric"

    print(f"  SC N=3: V={topo['V']}, E={topo['E']}, F={topo['F']}, C=27, "
          f"χ=0, 4 faces/edge OK")

    # FCC N=1
    topo = get_fcc_periodic_topology(1)
    assert topo['chi_3complex'] == 0, f"FCC χ(3)={topo['chi_3complex']}"
    assert topo['C'] == 4

    verts, edges, faces, cfi = build_fcc_supercell_periodic(1)
    # FCC: 3 faces per edge
    efc = defaultdict(int)
    for face in faces:
        n = len(face)
        for k in range(n):
            ...
            efc[e] += 1
    edge_counts = set(efc.values())
    assert edge_counts == {3}, f"FCC faces/edge: {edge_counts}, expected {{3}}"

    # Face-cell antisymmetry
    face_orients = defaultdict(list)
    for ci, cell in enumerate(cfi):
        for f_idx, orient in cell:
            face_orients[f_idx].append(orient)
    for f_idx, orients in face_orients.items():
        assert len(orients) == 2, f"FCC face {f_idx} in {len(orients)} cells"
        assert orients[0] + orients[1] == 0, f"FCC face {f_idx} not antisymmetric"

    # Each cell has 12 faces (rhombic dodecahedron)
    for ci, cell in enumerate(cfi):
        assert len(cell) == 12, f"FCC cell {ci}: {len(cell)} faces"

    # Edges canonical, no duplicates
    for i, j in edges:
        assert i < j, f"FCC edge ({i},{j})"
    assert len(set(edges)) == len(edges), "FCC duplicate edges"

    print(f"  FCC N=1: V={topo['V']}, E={topo['E']}, F={topo['F']}, C=4, "
          f"χ=0, 3 faces/edge, 12 faces/cell OK")
